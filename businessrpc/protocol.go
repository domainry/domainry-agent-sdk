...
var contractSHA = func() string {
	parts := []any{ProtocolVersion, CapabilitiesPath, CallPath, MaxRequestBytes, MaxResponseBytes, "trusted-scoped-delegation;current-execution-and-tool-policy;report-owner-authorization;preserve-json-numbers;no-retry;write-unknown-on-transport-failure", shape(reflect.TypeFor[request]()), shape(reflect.TypeFor[response]())}
