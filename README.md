...
- `persistence` owns Agent execution-state application ports, persistence capabilities and the shared redacted task-run view. Agent's HTTP surface and worker assembly consume `ExecutionStateBinding`; Runtime does not mutate Agent repositories or implement a second task/interactive state machine.
